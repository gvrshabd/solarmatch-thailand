...
type Lead = {
  ...
  selectionCompatible: boolean; contact_collection_mode: 'validation_interest'|'named_installer_handoff'; contact_configuration_version_id: string;
};
...
type ConfigurationPayload = {
  ...
  contacts: Array<{ id: string; version_number: number; state: string; contact_collection_mode: 'disabled'|'validation_interest'|'named_installer_handoff'; contact_collection_enabled: number; retention_days: number|null; receiving_company_en: string|null; receiving_company_th: string|null; receiving_company_privacy_url: string|null; permittedContactMethods: Array<'phone'|'line'>; sharedFields: string[]; created_by: string; created_at: string; published_at: string|null }>;
...
type ContactDraft = {
  mode: 'disabled'|'validation_interest'|'named_installer_handoff'; enabled: boolean; retentionDays: number|null;
...
type Tab = 'leads' | 'assessment' | 'scoring' | 'contact' | 'facts' | 'media' | 'history';
...
export function AdminDashboard({ csrfToken, signedInEmail }: { csrfToken: string; signedInEmail: string }) {
  ...
  }, [api, eligibility, exportScope, from, location, minimumAc, ownership, query, recipientKey, score, sort, status, to]);
  ...
  return <main className="admin-dashboard">
    ...
    <nav className="admin-tabs" aria-label="Administration sections">
      {([['leads', LayoutList, 'Leads'], ['assessment', Settings2, 'Assessment'], ['scoring', ShieldCheck, 'Scoring'], ['contact', ContactRound, 'Contact and consent'], ['facts', BookOpen, 'Solar facts'], ['media', ImageIcon, 'Media'], ['history', FileClock, 'History']] as const).map(([value, Icon, label]) => <button type="button" key={value} className={tab === value ? 'active' : ''} onClick={() => setTab(value)}><Icon />{label}</button>)}
    </nav>
    ...
    {tab === 'contact' && contactDraft && <section className="admin-section">
      ...
      <div className="admin-rule-grid admin-contact-grid">
        <label>Contact mode<select value={contactDraft.mode} onChange={(event) => { const mode = event.target.value as ContactDraft['mode']; setContactDraft({ ...contactDraft, mode, enabled: mode === 'disabled' ? false : contactDraft.enabled, receivingCompanyEn: mode === 'named_installer_handoff' ? contactDraft.receivingCompanyEn : null, receivingCompanyTh: mode === 'named_installer_handoff' ? contactDraft.receivingCompanyTh : null, receivingCompanyPrivacyUrl: mode === 'named_installer_handoff' ? contactDraft.receivingCompanyPrivacyUrl : null }); }}><option value="disabled">Disabled</option><option value="validation_interest">Validation interest — SolarMatch follow-up only</option><option value="named_installer_handoff">Named installer handoff</option></select></label>
      ...
      <div className="admin-score-preview"><h3>Public-copy preview</h3><p>{contactDraft.mode === 'disabled' ? 'No contact question or personal-information fields will appear.' : contactDraft.mode === 'validation_interest' ? 'SolarMatch asks permission for its own follow-up and explicitly states that data will not be shared with a solar company without separate permission.' : `The question names ${contactDraft.receivingCompanyEn || '[installer legal name]'} and links to the recipient’s Privacy Notice before consent.`}</p><div><article><strong>Activation status requested</strong><span>{contactDraft.enabled ? 'Enabled after readiness passes' : 'Disabled'}</span><small>Publishing an incomplete active configuration is rejected server-side.</small></article></div></div>
    </section>}

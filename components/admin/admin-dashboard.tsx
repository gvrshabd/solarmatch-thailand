...
type Lead = {
  ...
  submission_environment: 'production'|'private_development_preview'; is_test_submission: number; distribution_allowed: number; suppressed: number;
};
...
type ConfigurationPayload = {
  ...
  contacts: Array<{ id: string; version_number: number; state: string; contact_collection_mode: 'disabled'|'validation_interest'|'named_installer_handoff'|'shared_solar_company_handoff'; contact_collection_enabled: number; retention_days: number|null; distribution_window_days: number|null; recipient_category: string|null; adult_confirmation_version_id: string|null; consent_version_id: string|null; privacy_notice_version_id: string|null; terms_version_id: string|null; cookie_policy_version_id: string|null; internal_recipient_cap: number|null; receiving_company_en: string|null; receiving_company_th: string|null; receiving_company_privacy_url: string|null; permittedContactMethods: Array<'phone'|'line'>; sharedFields: string[]; created_by: string; created_at: string; published_at: string|null }>;
  ...
  privatePreview: { configured: boolean };
};

type ContactDraft = {
  mode: 'disabled'|'validation_interest'|'named_installer_handoff'|'shared_solar_company_handoff'; enabled: boolean; retentionDays: number|null;
...
type LeadDetailPayload = {
  lead: Record<string, unknown> & {
    ...
    submission_environment?: 'production'|'private_development_preview'; is_test_submission?: number; distribution_allowed?: number;
...
export function AdminDashboard({ csrfToken, signedInEmail }: { csrfToken: string; signedInEmail: string }) {
  ...
  return <main className="admin-dashboard">
    ...
    {configuration?.privatePreview.configured && <div className="admin-alert private-preview-admin" role="status"><strong>Private contact preview is active.</strong> Owner-authenticated submissions are marked as test records, suppressed, and blocked from partner selection, copying, export, and delivery.</div>}
    ...
    {tab === 'contact' && contactDraft && <section className="admin-section">
      ...
      <div className="admin-rule-grid admin-contact-grid">
        ...
        <label className="admin-checkbox"><input type="checkbox" checked={contactDraft.enabled} disabled={contactDraft.mode === 'disabled'} onChange={(event) => setContactDraft({ ...contactDraft, enabled: event.target.checked })} />Request activation when this draft is published</label>
      </div>

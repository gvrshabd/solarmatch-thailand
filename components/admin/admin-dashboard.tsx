...
type Lead = {
  ...
  selected: boolean; selectionReason: string; exportReason: string; explanation: { factors: Array<{ key: string; points: number; maximum: number; explanationEn: string }> };
  selectionCompatible: boolean; exportCompatible: boolean; contact_collection_mode: 'validation_interest'|'named_installer_handoff'|'shared_solar_company_handoff'; contact_configuration_version_id: string;
...
function leadClipboard(leads: Lead[]) {
  const lines = ['SolarMatch Thailand — selected residential leads', 'Administrative review copy — this action does not record or authorize delivery to a third party.', `Prepared: ${new Date().toLocaleString('en-GB', { timeZone: 'Asia/Bangkok' })}`, ''];
...
export function AdminDashboard({ csrfToken, signedInEmail }: { csrfToken: string; signedInEmail: string }) {
  ...
  async function changeSelection(lead: Lead, selectedValue: boolean) {
    await mutateLeads({ action: 'set-selection', leadId: lead.id, selection: selectedValue ? 'selected' : 'deselected' }, selectedValue ? 'Lead selected for administrator review.' : 'Lead deselected from administrator review.');
  }
  ...
  return <main className="admin-dashboard">
    ...
    {tab === 'contact' && contactDraft && <section className="admin-section">
      ...
      <div className="admin-rule-grid admin-contact-grid">
        <label>Contact mode<select value={contactDraft.mode} onChange={(event) => { const mode = event.target.value as ContactDraft['mode']; setContactDraft({ ...contactDraft, mode, enabled: mode === 'disabled' ? false : contactDraft.enabled, recipientCategory: mode === 'shared_solar_company_handoff' ? 'solar_service_recipients' : null, receivingCompanyEn: mode === 'named_installer_handoff' ? contactDraft.receivingCompanyEn : null, receivingCompanyTh: mode === 'named_installer_handoff' ? contactDraft.receivingCompanyTh : null, receivingCompanyPrivacyUrl: mode === 'named_installer_handoff' ? contactDraft.receivingCompanyPrivacyUrl : null }); }}><option value="disabled">Disabled</option><option value="validation_interest">SolarMatch follow-up only</option><option value="named_installer_handoff">Legacy named installer handoff</option><option value="shared_solar_company_handoff">Shared solar-service recipient handoff</option></select></label>

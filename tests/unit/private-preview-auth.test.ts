...
import { authenticatePrivatePreview, privatePreviewAccessConfiguration } from '@/lib/server/private-preview-auth';
...
const runtime: SolarMatchRuntimeEnv = {
  ...
  PRIVATE_CONTACT_PREVIEW_ENABLED: 'true',
...
describe('private contact-preview Access assertion', () => {
  it('requires the explicit kill switch and complete site-wide Access configuration', () => {
    expect(privatePreviewAccessConfiguration(runtime)).not.toBeNull();
    expect(privatePreviewAccessConfiguration({ ...runtime, PRIVATE_CONTACT_PREVIEW_ENABLED: 'false' })).toBeNull();
    expect(privatePreviewAccessConfiguration({ ...runtime, PRIVATE_SITE_ACCESS_AUD: undefined })).toBeNull();
    expect(privatePreviewAccessConfiguration({ ...runtime, ADMIN_EMAILS: '' })).toBeNull();
  });
  ...
  it('accepts only a current assertion for the site-wide AUD and exact allowlisted email', async () => {
    ...
    await expect(authenticatePrivatePreview(headers(), { runtime, verifyAssertion, nowMs })).resolves.toEqual({
  ...
  it.each([
    ...
    await expect(authenticatePrivatePreview(headers(), { runtime, verifyAssertion: async () => payload(override), nowMs })).resolves.toBeNull();
  });

  it('rejects missing and unverifiable assertions without exposing the error', async () => {
    await expect(authenticatePrivatePreview(new Headers(), { runtime, nowMs })).resolves.toBeNull();
    await expect(authenticatePrivatePreview(headers(), { runtime, verifyAssertion: async () => { throw new Error('invalid signature'); }, nowMs })).resolves.toBeNull();
  });
});

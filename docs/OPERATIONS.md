...
## Owner-only access-restricted operational collection

Access-restricted operational collection is available only when `PRIVATE_SITE_ACCESS_AUD` names the separate Worker-wide Access application, a current `Cf-Access-Jwt-Assertion` validates against that exact AUD and the configured team issuer, the asserted email is in the exact `ADMIN_EMAILS` allowlist, and the published contact configuration has `restricted_site_collection_enabled` enabled. The admin `ACCESS_AUD` remains unchanged and continues to protect `/admin*`.

While enabled, the authenticated owner can exercise the complete contact journey even though anonymous public collection remains disabled. Valid submissions are ordinary operational D1 records: `submission_environment = restricted_site_operational`, `is_test_submission = 0`, `access_restricted_submission = 1`, and `distribution_allowed = 1`. They appear under Residential contact submissions and remain subject to the normal consent, hard-eligibility, selection, partner, distribution-window, suppression, archive and purge controls. This does not activate a partner, invent an operator identity, alter public legal readiness, write contact data to R2, or allow anonymous access.

Before removing the temporary Worker-wide Access policy, keep `public_collection_enabled` disabled until the public legal/operator configuration is complete and explicitly approved for launch. Verify an anonymous `/api/assessment/config` response cannot issue a submission token while public collection is disabled. Then remove only the temporary Worker-wide policy. Keep `PRIVATE_SITE_ACCESS_AUD` harmlessly configured or remove it in a later reviewed code change. Never copy the temporary AUD into `ACCESS_AUD`, and never delete the `/admin*` application.

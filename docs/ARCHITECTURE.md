...
- Eleven required residential answers in `residential-questionnaire-v3`: province, bill, active installation planning, property type, ownership, roof area, daytime use, daytime appliances and conditional AC count, roof material, shade, and the integrated quote-contact decision. Immutable v1/v2 documents remain readable for historic records.
...
- Results are calculated locally before the integrated quote decision. Choosing No proceeds directly to the short result-preparation state without collecting PII. Choosing Yes requires the visible disclosure acknowledgement and opens one responsive contact-information panel; submission success, failure, or explicit skip still leads to the same complete result. Optional roof direction, slope, phase, map position, and other precision inputs refine an already complete estimate.
- Contact collection has three current fail-closed modes: disabled, SolarMatch-only validation follow-up, and shared residential-solar-company handoff. Historic named-installer records remain readable. Anonymous `/api/assessment/config` returns no signed submission token unless public collection passes the full legal/partner readiness gate.
- While the whole SolarMatch hostname remains behind its temporary Access gate, an exact allowlisted Access identity can use access-restricted operational collection. These are ordinary D1 submissions shown in the admin system, not preview rows. They are tagged `restricted_site_operational` and retain all normal consent, selection, distribution, archive and purge safeguards. Public collection remains independently disabled.
...
Migration `0005_operational_contact_flow.sql` separates owner-only restricted collection from future anonymous public collection, adds v3 planning/quote answers, permits LINE-only operational contact records, and preserves all existing lead rows plus child score, status, note, export, selection, delivery and privacy-request history during the constrained lead-table rebuild.
